...
DB_READY = True
DB_ERROR = ""
DB_CONFIG_KEYS = ("DATABASE_URL", "PG_DATABASE_URL", "PG_HOST", "PG_PORT", "PG_DB", "PG_USER", "PG_PASSWORD")
...
def database_target():
    ...
    if database_url:
        ...
        dbname = (parsed.path or "").lstrip("/") or "configured-db"
        user = parsed.username or "configured-user"
        return f"{host}:{port}/{dbname} as {user}"
    host = env_first("PG_HOST", default="localhost")
    port = env_first("PG_PORT", default="5432")
    dbname = env_first("PG_DB", default="kss_db")
    user = env_first("PG_USER", default="kss_user")
    return f"{host}:{port}/{dbname} as {user}"
...
def connect():
    ...
    connect_kwargs = {
        ...
        "connect_timeout": int(env_first("PG_CONNECT_TIMEOUT", "DB_CONNECT_TIMEOUT", default="5")),
    }
...
if __name__ == "__main__":
    try:
        ...
        if database_is_configured():
            print("PostgreSQL is configured but KSS could not connect, so it is starting in frontend-only mode.")
            print(f"Checked database: {database_target()}")
            print("Fix: start PostgreSQL on that host/port, or update .env to a reachable DATABASE_URL/PG_* database.")
        else:
